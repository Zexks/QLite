...
namespace QLite.Controls
{
    public class QTabs : TabControl
    {
        ...
        public QTabs()
            : base()
        {
            if (!this.DesignMode) this.SetStyle(ControlStyles.UserPaint | ControlStyles.AllPaintingInWmPaint | ControlStyles.Opaque | ControlStyles.ResizeRedraw, true);
        }
        ...
        public TabStyleProvider DisplayStyleProvider
        {
            get
            {
                if (this._StyleProvider == null) this.DisplayStyle = TabStyle.Default;
        ...
        public int ActiveIndex
        {
            get
            {
                TCHITTESTINFO hitTestInfo = new TCHITTESTINFO(this.PointToClient(Control.MousePosition));
                ...
                if (index != -1 && this.TabPages[index].Enabled) return index;
                else return -1;
            }
        }
        ...
        public TabPage ActiveTab
        {
            get
            {
                ...
                if (activeIndex > -1) return this.TabPages[activeIndex];
                else return null;
            }
        }
        ...
        {
            if (this.TabImageClick != null) this.TabImageClick(this, e);
        }

        protected virtual void OnTabClosing(TabControlCancelEventArgs e)
        {
            if (this.TabClosing != null) this.TabClosing(this, e);
        }
        ...
        protected virtual void OnHScroll(ScrollEventArgs e)
        {
            ...
            //	Raise the event
            if (this.HScroll != null) this.HScroll(this, e);
            if (e.Type == ScrollEventType.EndScroll) this._oldValue = e.NewValue;
        }
        ...
        public void Add(UserControl c)
        {
            TabPage tmp = new TabPage("Query");
            tmp.Controls.Add(c);
            this.TabPages.Add(tmp);
        }
        ...
        {
            if (this.IsValidTabIndex(index)) this.HideTab(this._TabPages[index]);
        }

        public void HideTab(string key)
        {
            if (this.TabPages.ContainsKey(key)) this.HideTab(this.TabPages[key]);
        }
        ...
        {
            if (this.IsValidTabIndex(index)) this.ShowTab(this._TabPages[index]);
        }
        ...
        private void CustomPaint(Graphics screenGraphics)
        {
            ...
            if (this.Width > 0 && this.Height > 0)
            {
                ...
                //	paint the tabs etc.
                if (this.RightToLeftLayout) screenGraphics.DrawImageUnscaled(this._BackBuffer, -1, 0);
                else screenGraphics.DrawImageUnscaled(this._BackBuffer, 0, 0);
            }
        }
        ...
        private void DrawTabBorder(GraphicsPath path, int index, Graphics graphics)
        {
            ...
            if (index == this.SelectedIndex) borderColor = this._StyleProvider.BorderColorSelected;
            else if (this._StyleProvider.HotTrack && index == this.ActiveIndex) borderColor = this._StyleProvider.BorderColorHot;
            else borderColor = this._StyleProvider.BorderColor;
        ...
        private void DrawTabText(int index, Graphics graphics)
        {
            ...
            else
            {
                if (this.TabPages[index].Enabled)
                    ...
                    { graphics.DrawString(this.TabPages[index].Text, this.Font, textBrush, tabBounds, this.GetStringFormat()); }
                else using (Brush textBrush = new SolidBrush(this._StyleProvider.TextColorDisabled))
                    { graphics.DrawString(this.TabPages[index].Text, this.Font, textBrush, tabBounds, this.GetStringFormat()); }
            }
        }
        ...
        private void BackupTabPages()
        {
            if (this._TabPages == null)
            {
                ...
                foreach (TabPage page in this.TabPages) this._TabPages.Add(page);
            }
        }

        private void StartDragDrop()
        {
            if (!this._dragStartPosition.IsEmpty && this.SelectedTab != null)
            {
        ...
        public override Rectangle DisplayRectangle
        {
            get
            {
                if (this._Style == TabStyle.None) return new Rectangle(0, 0, Width, Height);
                else
                {
                    int tabStripHeight = 0;
                    int itemHeight = 0;

                    if (this.Alignment <= TabAlignment.Bottom) itemHeight = this.ItemSize.Height;
                    else itemHeight = this.ItemSize.Width;
        ...
        protected override CreateParams CreateParams
        {
            ...
            get
            {
                ...
                if (this.RightToLeftLayout) cp.ExStyle = cp.ExStyle | NativeMethods.WS_EX_LAYOUTRTL | NativeMethods.WS_EX_NOINHERITLAYOUT;
                return cp;
            }
        }
        ...
        {
            if (e.ClipRectangle.Equals(this.ClientRectangle)) this.CustomPaint(e.Graphics);
        ...
        protected override void OnResize(EventArgs e)
        {
            if (this.Width > 0 && this.Height > 0)
            {
                ...
                if (this._BackBufferGraphics != null) this._BackBufferGraphics.Dispose();
                if (this._BackBuffer != null) this._BackBuffer.Dispose();

                this._BackBuffer = new Bitmap(this.Width, this.Height);
                this._BackBufferGraphics = Graphics.FromImage(this._BackBuffer);

                if (this._TabBufferGraphics != null) this._TabBufferGraphics.Dispose();
                if (this._TabBuffer != null) this._TabBuffer.Dispose();
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (this._StyleProvider.ShowTabCloser)
            {
                ...
                if (tabRect.Contains(this.MousePosition)) this.Invalidate();
            }

            if (this.AllowDrop && e.Button == MouseButtons.Left) this.StartDragDrop();
        }

        protected override void OnMouseDown(MouseEventArgs e)
        {
            base.OnMouseDown(e);
            if (this.AllowDrop) this._dragStartPosition = new Point(e.X, e.Y);
        }

        protected override void OnMouseUp(MouseEventArgs e)
        {
            base.OnMouseUp(e);
            if (this.AllowDrop) this._dragStartPosition = Point.Empty;
        }
        ...
        protected override void OnDragOver(DragEventArgs drgevent)
        {
            ...
            if (drgevent.Data.GetDataPresent(typeof(TabPage))) drgevent.Effect = DragDropEffects.Move;
        ...
        protected override void OnDragDrop(DragEventArgs drgevent)
        {
            ...
            if (drgevent.Data.GetDataPresent(typeof(TabPage)))
            {
                ...
                if (this.ActiveTab == dragTab) return;

                int insertPoint = this.ActiveIndex;
                if (dragTab.Parent.Equals(this) && this.TabPages.IndexOf(dragTab) < insertPoint) insertPoint--;
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                if (this._BackImage != null) this._BackImage.Dispose();
                if (this._BackBufferGraphics != null) this._BackBufferGraphics.Dispose();
                if (this._BackBuffer != null) this._BackBuffer.Dispose();
                if (this._TabBufferGraphics != null) this._TabBufferGraphics.Dispose();
                if (this._TabBuffer != null) this._TabBuffer.Dispose();
                if (this._StyleProvider != null) this._StyleProvider.Dispose();
            }
        }
